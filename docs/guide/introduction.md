...
import AppSource from '../examples/intro/App.vue?raw'
import HelloSource from '../examples/intro/HelloWorld.vue?raw'
import LayersSource from '../examples/intro/layers.ts?raw'
</script>
...
点一下就能打开（内容里用 defineLayer 加了标题，点右上角关闭即可）：

<DemoBlock
  :demo="Demo"
  :files="[
    { name: 'App.vue', code: AppSource },
    { name: 'HelloWorld.vue', code: HelloSource },
...
3. **进阶** — 配置合并、响应式、adapter、实例细节、SSR  
4. [API](/api/) — 查签名与类型  
5. （可选）[实践](/guide/cookbook/) — 按场景查：复用内容、获取结果、存量接入、嵌套等  

本地可跑导航栏里的 **Playground** 对照更多示例。
