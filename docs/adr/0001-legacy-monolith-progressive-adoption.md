...
| API | 用途 |
|-----|------|
| **`LayerNoContainer`** | 标记「无外层 Dialog」；与普通容器同构（Teleport + default 锚点），并把 `container.props` 投影到 content |
...
- `LayerNoContainer` 只透传 default slot（`inheritAttrs: false`，容器上不挂 model）；`modelValue`、create 默认 title/width 等经 **props 投影**落到 content（content 覆盖 container）。
- 正常用法不要在 `content.props` 里传 `modelValue`（否则会盖掉框架可见性；视为故意覆盖）。
- `container.slots`（含 container 侧 `LayerTemplate`）挂在透明容器上，**无有效 UI**（与「无外层 Dialog」一致）。
...
| 叙事 | 含义 | 推荐写法 |
|------|------|----------|
| **仅容器** | `useX()` 未绑 Content；层合法，只是没有业务体 | 配容器用 `open` / `use` 的 **`container:`**（content 取向 flat 顶层进 content 列；无 `content.component` 时该列配置丢弃） |
| **单体接入** | 内嵌 dialog 的存量组件 | **`LayerNoContainer` + 单体做 content**（或 `createLayer(BaseDialog)` + adapter 换成 NoContainer） |

因此：

- **无 content ≠ 单体场景。** 仅容器 是 split 下的兼容用法，不是「把单体放进 container、视 content 不存在」。
- **单体不要当 `createLayer` 第一参。** 那样仍把单体当「容器」走 split，角色与 L3「单体 = content」相反；应 `LayerNoContainer` + 单体 content。
- **props 投影不是对称先例。** `LayerNoContainer` 将 `container.props`（含 create 默认与 bind 的 `model`）摊到 content，是为了混用工厂与可见性协议在无 Dialog 容器时仍成立；**不**据此要求「无 content 时把 content 列配置摊到 container」。
- **不做 `LayerNoContent`。** 不为「content 列配置落到 container vnode」提供反向标记；仅容器 显式写 `container:` 即可。
...
| 不公开标记、仅内部哨兵 | 否决——项目 adapter 无法换容器，无法共用 `useLayer` |
| 单体作 container | 否决——L3 角色反了；亦非仅容器 的正确建模 |
| **`LayerNoContent` / 反向投影** | 否决——与仅容器 / 单体两套叙事混淆；容器配置已有 `container:`；无「管线写在 content 却必须喂容器」的对等负担 |
...
- 仅容器（无 Content）：实例可 open/close；容器 props / slots 经 **`container:`** 写入

### 弱化 / 无效（用错自负）

| 项 | 说明 |
|----|------|
| `defineLayer` 改 container props | 透明容器无 Dialog UI 可挂；页内/误用时行为与「无有效容器 UI」一致 |
| `LayerTemplate` `container` | 透明容器不渲染有意义的 container slots |
| `content.props.modelValue` | 会覆盖 bind 投影 |
| 仅容器 时顶层 `open({ props })` | 进 content 列；无 `content.component` 时丢弃——应改 `container: { props }` |
...
- 无第二套类型 / 工厂。
- 实现为 `LayerNoContainer` 透明容器 + props 投影（与普通容器同 Teleport 树）。

### 限制

- `withDialog`（或等价标记）由**项目**维护。
- 透明容器下 container 侧模板 / define 容器 UI 无意义。
- 仅容器 与单体接入是两套叙事；无 `LayerNoContent`，不为 content→container 做对称投影。
- 仅容器 与单体接入是两套叙事；无 `LayerNoContent`，不为 content→container 做对称投影。
