...
### 1. 哪些 API 接受 `LayerConfigSource`

```ts
type LayerConfigSource<T> = T | Ref<T> | (() => T) // 与 Vue MaybeRefOrGetter 对齐
```

| API | 配置源 | 语义 |
|-----|--------|------|
| `createLayer(Container, config?)` | `LayerConfigSource<LayerConfigCreate>`（含可选 `adapter`） | `create` tier live；见 §5 |
| `defineLayer(config?)` | `LayerConfigSource<LayerConfigStatic>` | `define` tier live |
| `useLayer(Content?, config?)` | `LayerConfigSource<LayerConfigInstance>` | `use` tier live；`Content` 第一参保持静态 `Component` |
| `clone(config?)` | `LayerConfigSource<LayerConfigInstance>` | 见 §3 |
...
```ts
clone(config: LayerConfigSource<LayerConfigInstance> = {}) {
  use: computed(() =>
    mergeFragment(
...
与「工厂创建时抠出 `adapter` 另传 prop」不同：**`adapter` 跟其它 create 字段走同一条 `LayerConfigSource → computed → store.create`**，在 merge 之后、bind 之前取出使用。
...
| 项 | 结论 |
|----|------|
| `create` / `define` / `use` / `clone` 配置源 | `LayerConfigSource`（MaybeRefOrGetter） |
| `open` | 仅 plain 快照 |
