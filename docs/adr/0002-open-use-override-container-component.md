...
[ADR 0001](./0001-legacy-monolith-progressive-adoption.md) 定稿：存量单体通过把 `container.component` 换成 **`LayerNoContainer`**（透明容器 + props 投影；常见于项目 **adapter** 按 content 组件切换）；也允许在 `use` / `open` 显式传入。
...
换容器后的 slot 名、`model` 协议等差异由用户在 **adapter** 或分工厂处理；框架不替用户适配。

覆盖为 `LayerNoContainer` 时走 [ADR 0001](./0001-legacy-monolith-progressive-adoption.md) 透明容器 + props 投影。
...
曾考虑「允许 + warn」以提示非常规换容器。否决原因：

- 类型与 merge 已把它当作一等字段；单独 warn 不一致
- 合法场景多（单实例换容器、显式 `LayerNoContainer`、个别页 Drawer）
...
| 主题 | ADR |
|------|-----|
| `LayerNoContainer` 透明容器 / props 投影、共用 `useLayer` | **0001** |
