...
```
CreateDialog ≈ CreateForm + useDialog.layer + LayerSlot
```
...
content 内声明 layer 配置，footer 等 layer 插槽用 `LayerSlot` co-locate；content 自己的 slot 正常写在 template 里。
...
LayerRoot
  └─ layer（ElDialog）
       ...
       └─ footer 等 ← layer slots（LayerSlot）
```
...
```ts
{
  ...
  slots?: { [name]: ref }      // content 组件 slot → LayerSlot ref
...
底层用同一套机制（`LayerSlot` + ref + `render()`），但挂载目标不同：
...
标记将被 slot 配置引用的内容。不在对应渲染上下文内时不输出 DOM。
