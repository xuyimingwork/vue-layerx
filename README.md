...
```ts
// 模块级：绑定外壳组件
const useDialog = createLayerx(ElDialog, { props: { width: '520px' } })
...
```
createLayerx(Shell, shellDefaults)
        │
        ▼
...
```
src/
  create-layerx.ts       # 对外工厂 createLayerx
  use-layer.ts           # useLayer 编排（状态、show/hide）
  define-layer-component.ts
  build-vnode.ts
  body-renderer.ts
  shell-config.ts
  types.ts
  utils/                 # mergeProps、bindCloseOn、collectSlots
```

## API（当前）

### `createLayerx(Shell, options?)`
...
**两种用法均支持**：
- **不写 template**：`show()` 将弹层渲染到 `document.body`（继承 `appContext`）
- **写 template**：`<XxxDialog />` 在组件树内渲染，可透传响应式 attrs、插槽、`@event`；`show()` 同样可用
