...
- **merge**：各层贡献 **LayerNodeConfig 片段**（`content` / `layer`）与 `hideOn`；优先级固定。`layerTemplates` / `contentTemplates` **不经 merge**，按 **`LayerTemplate` 路由**（无 `to` → layer 链，有 `to` → content 链），在 resolve 读取（见「渲染与投递机制」）。
- **resolve**：先物化 `layerTemplates` / `contentTemplates` 为 `normalized.layer.slots` / `normalized.content.slots`；再用 merge 阶段已按优先级合并好的**命令式** `layer.slots` / `content.slots`（`SlotRenderFn`）**同 key 覆盖**物化结果；`hideOn` 接入 content 事件；产出 **LayerNormalized**。
- **adapt**：在 **LayerNormalized** 上整形（滤 props、搬移 / 重命名 `layer.slots` 的 key、**可改 `layer.component`** 等）；`show` 已反映在入参中。实例由哪个工厂创建，就跑该工厂 `createLayer` 注册时的**那一个** `adapt`（不按 `show` 里的 `layer.component` 动态换 adapt）。
...
`show` 可覆盖 merge 输入（含 `component`、`layer.component`），但**不跳过 adapt**——仍走 merge → resolve → adapt → render；adapt 可改回或替换 `layer.component`。
...
`LayerTemplate` 在 mount 时向实例注册表写入 `{ name, render }`；**resolve 阶段物化为 `SlotRenderFn`**，由框架在 `h(layer, props, slots)` / `h(content, props, slots)` 时传入。四段渲染顺序：
...
- 无 `:to` 的 `LayerTemplate`（UserForm 内）写入实例的 **`layerTemplates`**，resolve 时物化后作为 MyDialog 的 slot fn 传入。
...
|------|--------|-------------------|
| UserForm 内 `LayerTemplate`（无 `to`） | `normalized.layer.slots` → **MyDialog** | 否 |
...
- UserList 写 `name="footer"` 且带 `:to`，而 UserForm 只有 layer 侧 `LayerTemplate name="footer"`、没有 `<slot name="footer">`：内容走 **content** 链，对不上口 → **不展示**；**换不掉** MyDialog 上的 layer footer（那条链 UserList 触及不到）。
...
| **defineLayer** | ✅ | content 贡献 `layer.props`；极少用 `layer.slots` 覆盖模板 | 选 `component`、适配容器 |
| **useX(Content, opts?)** | ✅ | 使用侧片段、`hideOn`、可绑 Content | 适配 MyDialog |
| **show(payload?)** | ✅ 最高 | 可覆盖 merge 一切，含 `component`、`layer.component` | 绕过 adapt（仍须走 adapt；adapt 可改回 component） |
| **defaultResolve** | — | `LayerMerged` → `LayerNormalized` | 不参与优先级 |
| **createLayer 第 3 参 adapt** | — | `LayerNormalized` → `LayerNormalized` | 不实现 merge |
| **LayerTemplate**（UserForm 内，无 `to`） | layerTemplates（resolve 读取） | 注册 layer 侧模板内容 | — |
...
| 方式 | 典型用途 | 优先级 |
|------|----------|--------|
| `LayerTemplate name="footer"` | 声明 layer 侧块级模板（`name` = MyDialog slot 名） | 默认 |
| `defineLayer({ layer: { slots: { title: () => h(...) } } })` | 命令式覆盖同名 slot 渲染（少见） | **高于** `LayerTemplate` |

正常路径：**`LayerTemplate name` 与 layer / content 的 slot 同名**。`defineLayer` 一般只写 `props`；需命令式覆盖时再写 `layer.slots`。
...
`useX` 由 `createLayer` 返回。`options` 与 `show` payload **同构**，形状为 **content 节点配置 + `layer` + `hideOn`**：

```ts
/** 顶层字段即 content（LayerNodeConfig）；另附 layer 与 hideOn */
...
即 `{ component?, props?, slots? }` 描述 **content**；`layer` 描述外层容器片段。`useX(UserForm, { props })` 中的 `props` 即 `content.props`。顶层 / `layer.slots` 仅用于极少见的命令式 `SlotRenderFn`；常规插槽内容用 `LayerTemplate`。
...
`show` 可改 `layer.component`；merge → resolve 后仍走**创建该实例的工厂**所注册的 `adapt`（如 `useDialog` 的 adapt）。`adapt` 收到完整 `LayerNormalized`，**可改回或改成别的 `layer.component`**，以 adapt 返回为准。常规双容器仍推荐 `useDialog` / `useDrawer` 分工厂。

**`show` 换 `layer.component`** 属进阶能力：框架只提供 merge + adapt 钩子，**不替用户处理**容器差异（slot 名、visible 协议等）；非常规需求下用户须在 `adapt` 内自行处理，或对默认 Layer 做二次封装。
...
const wide = base.clone({ layer: { props: { width: '640px' } } })

wide.show({ props: { id: 1 } })
// merge：show.props > partial.layer.props.width > useX.hideOn > …
```
...
对 `content.*`、`layer.*` 各字段分别 merge（浅合并 `props`；命令式 `slots`（`SlotRenderFn`）同 key 后者覆盖前者，结果写入 `LayerMerged.content.slots` / `LayerMerged.layer.slots`，在 **resolve** 阶段覆盖模板物化结果）。

`layerTemplates` / `contentTemplates` **不参与**上述优先级链，由声明位置在 resolve 时先物化，再被 merge 好的命令式 slots 同 key 覆盖。

### merge 后字段来源示例

**layer.props**（`defineLayer` 的顶层 `props` 简写 ≡ `layer.props`）：

```text
createLayer.layer.props
...
`show` 写入的 `layer.component`、`layer.props` 等 **先 merge → resolve → adapt**（始终为该实例所属工厂的**唯一** `adapt`）。容器 slot 名差异、换 `layer.component` 等均在 **adapt** 内处理；adapt 返回值即最终 `LayerNormalized`。

---

## 类型提示（可选）

框架**能**从 `createLayer` 推导 `layer.props`、从 content 组件推导 `props` / `emits`（进而约束 `hideOn`）。  
**不能**从 `UserForm` 自动读出有哪些 layer slot、content slot——与 Vue 父组件无法自省子组件 slot 开口一样。

`useX()` **未绑 Content** 时，layer 可正常显隐；`show({ component })` 传入 content 后类型推导生效。`props` / `layer.props` 等**推不出则回落 `any`**（不为此做复杂条件类型）。
...
| layer 自带关闭 | 内部 `hide()` |
| `beforeClose` | 写在 `layer.props`，经 merge/adapt **透传给底层 layer 组件**；属于容器自身行为，**框架不介入**，与 `hideOn` 无关 |
...
10. **`to` 分流**：无 `to` → layer 链；有 `to` → content 链；不接 MyDialog。
