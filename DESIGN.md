...
- **merge**：各层贡献 **LayerNodeConfig 片段**（`content` / `layer`）与 `hideOn`；优先级固定。`layerTemplates` / `contentTemplates` **不经 merge**，按**声明位置**分流，在 resolve 读取（见「渲染与投递机制」）。
